...
const positionMap = computed(() =>
  new Map(positions.value.map((position) => [position.id, position])),
)
  ...
  >
    <template v-if="mode === 'aside'">
      ...
      <button
        ...
        @click="openModal"
      >
        <svg viewBox="0 0 240 150" role="img" :aria-label="`Local graph for ${currentTitle}`">
          ...
          <g class="ayn-local-graph-preview-nodes">
            ...
            >
              <title>{{ nodeMap.get(position.id)?.title }}</title>
            </circle>
          </g>
        </svg>
