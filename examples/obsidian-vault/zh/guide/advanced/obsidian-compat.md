...
| `==高亮==` | `<mark>` |
| `%%comment%%` | 正文不显示;**默认保留为 `<!-- HTML 注释 -->`**(`preserveAsHtmlComment` 默认 true,可关) |
...
| 项 | 状况 | 备注 |
|---|---|---|
| `[[note#^block-id]]` 跨页跳 block-ref | ⚠️ 部分 | 块锚点本身可用:`^id` 已写进 DOM,**浏览器原生 `页面#^id` 跳转有效**。但 wikilink 形式 `[[note#^id]]` resolver 还**不识别** `#^` 前缀,会当未命中锚点;跨页跳 block 请用普通链接 `[文字](/path#^id)` |
| 数学 `$x^2$` / `$$...$$` | ⚠️ 用户启用 | VitePress 内置 `markdown.math: true` 选项(需要装 `markdown-it-mathjax3`) |
| `cssclasses:` frontmatter | ❌ | **暂未实现**(Obsidian 有,本插件还没读这个字段)。要给页面加 class 目前请用 VitePress 原生 `pageClass` frontmatter |
...
| Graph view | 自带 | 自带([[views|三大视图]]) |
| Dataview | 部分 | 暂不支持(路线计划) |
