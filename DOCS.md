...
sidebarAuto?: {
  ...
  // v0.3.10 — autoFolderIndex was removed. Folder URLs now resolve via folderLinkOrder.
  ...
  /**
   * v0.3.10: order in which folder link is resolved. The first hit wins.
   * Tokens:
  ...
  /**
   * @deprecated v0.3.10. Use `folderLinkOrder` instead.
...
### Folder URLs (v0.3.10 — no more index file generation)
...
- **dev hot-reload**: changing the vault tree (add/remove file, edit `_sidebar.md`)
  now **auto-restarts** the dev server in v0.3.10. Edits to file content (no rename)
  still HMR normally.
...
export default defineConfigWithAllYouNeed(
  ...
  // Every field below shows the DEFAULT value. Delete what you don't need to override.
  {
    ...
    sidebarAuto: {
      ...
      autoNav: false,
      homeNavText: 'Home',
      // autoFolderIndex was REMOVED in v0.3.10 — folder URLs now resolve via folderLinkOrder.
      ...
      folderLinkOrder: ['same-name', 'index', 'readme', 'first-file'],  // v0.3.10
      // folderLinkFallback: 'first-file',   // @deprecated v0.3.10 — use folderLinkOrder
      hiddenKey: 'sidebarHidden',
      titleKey:  'sidebarTitle',
      orderKey:  'order',
